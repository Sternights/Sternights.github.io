...
![https://developers.weixin.qq.com/miniprogram/dev/devtools/download.html?t=2018412](../img/post3-01.png)
...
- `pages` 字段 —— 用于描述当前小程序所有页面路径，这就是为了让微信客户端知道当前你的小程序页面定义
在哪个目录。
...
---
到这里，你可以自己试着添加一些组件和更改组件的样式，小程序的组件都有很多好玩的功能，像`camera`，`map`等，以下网址包括了所有组件

https://developers.weixin.qq.com/miniprogram/dev/component/map.html#map

还有小程序还提供了很多强大的API，如图片`wx.chooseImage`，录音`wx.startRecord`，获取位置`wx.getLocation`，还有一个很强大的API —— 扫码`wx.scanCode`，以下网址包括了所有组件
